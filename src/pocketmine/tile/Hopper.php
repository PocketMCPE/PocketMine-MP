...
class Hopper extends Spawnable implements InventoryHolder, Container, Nameable{
    ...
    /** @var array */
    protected static $processedEntities = [];
    ...
    public function onUpdate(){
        ...
        // Clear processed entities from previous tick
        if(count(self::$processedEntities) > 0) {
            self::$processedEntities = [];
        }
        ...
        foreach($this->getLevel()->getChunkEntities($this->getBlock()->x >> 4, $this->getBlock()->z >> 4) as $entity){
            ...
            // Check if entity has already been processed in this tick
            $entityId = $entity->getId();
            if(isset(self::$processedEntities[$entityId])){
                continue;
            }
            ...
            if($this->inventory->canAddItem($item)){
                // Mark entity as processed before adding to inventory
                self::$processedEntities[$entityId] = true;

                // Clone the item to avoid reference issues
                $clonedItem = clone $item;
                $this->inventory->addItem($clonedItem);

                // Remove the entity before processing others
                $entity->close();
            }
        }
        ...
        if($source instanceof Tile and $source instanceof InventoryHolder){
            ...
            $firstOccupied = $inventory->firstOccupied();
            if($firstOccupied !== -1){
